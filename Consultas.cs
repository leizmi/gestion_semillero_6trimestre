...
namespace gestión_semillero_6trimestre
{
    ...
    internal class Consultas
    {
        ...
        public Boolean Iniciar_sesion(int ID_usuario, string contraseña_usuario)
        {
            ...
            consulta = new SqlCommand("select ID_usuario,contraseña_usuario,tipo_usuario, estado_usuario from usuario where ID_usuario = @ID_usuario and contraseña_usuario = @contraseña_usuario", conexion.Conectar());//establecemos la consulta SQL para verificar el usuario y la contraseña
            ...
            try
            {
                ...
                if (Convert.ToString(ID_usuario) == dr["ID_usuario"].ToString() && contraseña_usuario == dr["contraseña_usuario"].ToString() && "Administrador" == dr["tipo_usuario"].ToString() && "activo" == dr["estado_usuario"].ToString()) // verificamos si el ID_usuario y el contraseña_usuario ingresados por el usuario coinciden con los resultados de la consulta a la base de datos
                {
                    MessageBox.Show("Bienvenido Administrador"); // si el id_usuario y contraseña ingresados por el usuario coinciden con los resultados de la consulta a la base de datos, se muestra un mensaje de bienvenida al usuario
                ...
                {
                    if (Convert.ToString(ID_usuario) == dr["ID_usuario"].ToString() && contraseña_usuario == dr["contraseña_usuario"].ToString() && "Lider" == dr["tipo_usuario"].ToString() && "activo" == dr["estado_usuario"].ToString()) // verificamos si el ID_usuario y el contraseña_usuario ingresados por el usuario coinciden con los resultados de la consulta a la base de datos
                    {
